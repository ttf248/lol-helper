...
flowchart TB
    ...
    subgraph History["⑤ 历史战绩拉取（LCU → SGP → 本地缓存）"]
        ...
        H6["来源标签:<br/>lcu-current / lcu-puuid / sgp / mixed<br/>同时记录 endpoints 明细（接口路径）"]:::proc
    end
...
3. **PostgreSQL 只做缓存，不做权威源**。`DatabaseState::initialize` 在 Tauri 启动期建表，前端用 `cacheHistory` 写入最近三页、`getCachedHistory` 按 `puuid + modeKey` 读回，再由 `mergeHistoryGames`（`recentAnalytics.ts`）按 `gameId` 与服务器结果合并去重。
